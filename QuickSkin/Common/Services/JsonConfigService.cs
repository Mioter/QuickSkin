    ...
    where T : new()
{
    ...
    {
        SavePath = savePath ?? Path.Combine(Directory.GetCurrentDirectory(), "configs");
        FileExtension = fileExtension ?? ".QwQ.json";
        _fullPath = Path.Combine(SavePath, $"{fileName}{FileExtension}");
    ...
    public void Save(T data)
    {
        ...
        {
            EnsureDirectory();
            string json = JsonSerializer.Serialize(data, typeof(T), _jsonSerializerContext);
    ...
    public T Load()
    {
        try
        {
            ...
            var jsonTypeInfo = (JsonTypeInfo<T>)_jsonSerializerContext.GetTypeInfo(typeof(T))!;
